 ...
 *   3. take the challenge's external scripts out of the proxy's captures
 *   4. POST them to `/dd/solve` from Node — direct, not through the proxy
 *   5. submit from *inside the challenge frame* with `fetch()`, so it carries
 *      the browser's own connection, headers and cookies
 *   6. set the clearance cookie and navigate again
 *
 * Step 5 is the part worth keeping whatever the browser: DataDome binds the
 * cookie to the IP that submitted it, and every request Lightpanda makes goes
 * through the proxy it was started with.
 *
 * Step 3 is newer. DataDome moved the collector out of the challenge document
 * and into a `<script src>` on captcha-delivery.com, and `/dd/solve` answers
 * `422 dd.script.missing` without it — which is what this script did until the
 * proxy started keeping every captcha-delivery.com response rather than only
 * the documents.
 */
...
const attempt = async ({
  ...
  //
  // Every captcha-delivery.com response is kept, not just the document. A
  // challenge whose collector is a `<script src>` needs those script bodies
  // sent alongside it or `/dd/solve` answers `422 dd.script.missing`, and the
  ...
  try {
    ...
    const documentHtml = captures.get(documentUrl);
    if (!documentHtml) {
      const seen = [...captures.keys()].filter(isDocument).length;
      throw new Error(
        `the proxy never saw ${documentUrl} — it captured ${seen} challenge documents`
      );
    }
    ...
    // 3. Collect the challenge's external scripts. Newer challenges keep the
    //    collector in a `<script src>` on captcha-delivery.com rather than
    //    inline, and `/dd/solve` rejects a submission that does not carry
    //    them. Prefer the proxy's capture — those are the bytes the page
    //    itself ran — and refetch only what it somehow missed, through the
    //    same proxy and exit IP so nothing about the request changes.
    ...
    for (const url of extractExternalScriptUrls(documentHtml, documentUrl)) {
      ...
      if (!mitm) throw new Error(`the proxy never saw ${url}`);
      log(`GET external script ${url}`);
      const fetched = await mitm.fetch({
        headers: {
          accept: '*/*',
          referer: documentUrl,
      ...
      log(`  <- HTTP ${fetched.status} (${fetched.body.length} bytes)`);
      if (fetched.status < 200 || fetched.status >= 300) {
        throw new Error(
          `external script returned HTTP ${fetched.status}: ${url}`
        );
      }
      externalScripts.push({ body: fetched.body, url });
    }

    // 4. Ask xhr.dev to build the submission — but not to send it.
    log('POST /dd/solve');
    ...
    // 5. Submit from inside the challenge frame. Same origin as the endpoint,
    ...
    // 6. Put the cookie in the jar, replacing rather than adding to it. The
    ...
    // 7. Prove it: the navigation that 403'd should now return the real page.
