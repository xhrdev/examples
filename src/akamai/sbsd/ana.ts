...
const settle = async (): Promise<void> => {
  ...
  const answeredHere = (): number =>
    akamai?.realms().find((r) => r.host === host)?.answered ?? 0;
  while (
...
const solveRealms = async (): Promise<void> => {
  if (!akamai) return;
  const solved = await akamai.solveAll({ waitForAcceptance: false });
  log(`realms solved: ${solved.join(', ') || 'none'}`);
};
...
try {
  ...
  /* Started and left running, not awaited.
   *
   * `waitForAcceptance: false` returns on SILENCE — an idle timer 15s after the
   * last answered round — and `www` has never reached `~0~` in any session
   * measured. Awaiting it cost ~40s before the form was touched, for a cookie
   * that does not move. The acceptance that matters is on the booking engine,
   * after the search, where it takes two rounds. */
  void solveRealms();
